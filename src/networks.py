...
class DepResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DepResidualBlock, self).__init__()
        # 通道逐点卷积
        self.pointwise_conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=1)
        # 5x5通道逐通道深度可分离卷积
        self.depthwise_conv1 = nn.Conv2d(out_channels, out_channels, kernel_size=5, padding=2, groups=out_channels)
        # 7x7通道深度可分离卷积
        self.depthwise_conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=7, padding=3, groups=out_channels)
        # 通道逐点卷积
...
class HazeRemovalNet(BaseNetwork):
    def __init__(self, base_channel_nums, init_weights=True, path=None, min_beta=0.04, max_beta=0.2, min_d=0.3, max_d=5, use_dc_A=False):
        ...
        in_channels = 32  # 根据你的网络架构选择适当的输入通道数
        out_channels = 32  # 根据你的网络架构选择适当的输出通道数
        
        in_channels_1 = 64  # 根据你的网络架构选择适当的输入通道数
        out_channels_1 = 64  # 根据你的网络架构选择适当的输出通道数

        #in_channels_2 = 128  # 根据你的网络架构选择适当的输入通道数
        #out_channels_2 = 128  # 根据你的网络架构选择适当的输出通道数

        self.dep_residual_block = DepResidualBlock(in_channels, out_channels)
        self.dep_residual_block_1 = DepResidualBlock(in_channels_1, out_channels_1)
    ...
    def forward(self, x_0, require_paras=False, use_guided_filter=False):
        ...
        out1 = self.dep_residual_block(layer_1)#第一次
        ...
        layer_1_rn=layer_1_rn+out1#第一次
        layer_2_rn = self.scratch.layer2_rn(layer_2)
        out2 = self.dep_residual_block_1(layer_2_rn)#第二次
        layer_2_rn= layer_2_rn+out2
        ...
        path_3=path_3+out2#第二次
        path_2 = self.scratch.refinenet2(path_3, layer_2_rn)
        path_2=path_2+out1#第一次
        path_1 = self.scratch.refinenet1(path_2, layer_1_rn)
        model = HFRM(in_channels=32, out_channels=32).cuda()
        path_1 = model(path_1)
