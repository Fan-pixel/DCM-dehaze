...
class D4():
    def __init__(self, config):
        ...
        self.results_path = os.path.join(config.PATH, 'result')
        ...
        with torch.no_grad():
            ...
            if model == 1:
                ...
                images_sample = stitch_images(
                    ...
                    self.postprocess(pred_t),#对透射率进行后处理，得到处理后的透射率图像。
                    img_per_row=1
                )
...
#实现了图片后处理的功能,将图像的像素值从范围[0, 1]映射到[0, 255]，并转换为整型数据类型。
    ...
    def generate_color_map(self, imgs, size=[256,256]):
        ...
        return colormaps
#对给定的结果图像进行裁剪，以使其高度和宽度都能够被指定的倍数（times）整除。
    def crop_result(self, result, input_h, input_w, times=32):
        ...
        return result
#对输入的张量（tensor）进行填充，使其高度和宽度都能被指定的倍数（times）整除。
